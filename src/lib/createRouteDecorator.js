...
export default function createRouteDecorator(store: Object): (routes: any) => any {
  ...
  function decorateIndexRoute(route: any): any {
    const result = decorateRoutes(route)
    if (Array.isArray(result)) {
      if (result.length !== 1) throw new Error("got more than one route for an index route: " + route)
      return result[0]
    }
    return result
  }
  ...
  function wrapGetIndexRoute(func: Function, ...args: any[]): any {
    return (location, cb) => {
      const result = func(location, ...args, (err, routes) => {
        if (err) return cb(err)
        return cb(null, decorateIndexRoute(routes)[0])
      })
      if (result) return decorateIndexRoute(result)[0]
    }
  }
